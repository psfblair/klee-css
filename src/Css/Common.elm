...
{-| Types that represent records of functions for implementing common values shared between
multiple CSS properties, like `auto`, `inherit`, `none`, `normal` and several more.
These are used by modules within elm-css and not intended to be used as part of the Css DSL.

Implementation notes:

In Haskell's Clay, the use of type classes and GeneralizedNewtypeDeriving allows types such as Stroke
and Size to derive from the default implementations of the typeclasses Val, Other, Inherit, Auto, and None.
This makes the Haskell code quite succinct.

In elm-css we instead pass around records of functions to wrap the types. These are referred to as
factories. Example:
...
and then the argument passed to borderStroke will be a strokeFactory, and borderStroke
will then call the factory and wrap the result in the appropriate union type constructor.
The factory is a record type that contains functions for all the various types that need to
be wrapped; i.e.
...
none that turns a None into a value, so that when it comes time to convert the Stroke to
a Value, we have a ValueFactory Stroke which looks like this:
...
Both produce an instance of the Stroke union type. This is made possible by structural typing:
The StrokeFactory record type has a field none, which also makes that record of type None.
Thus, the none function above, which takes a record of type None, will accept a record
of type StrokeFactory. This means that the none function can be passed to borderStroke, which
requires a function that takes a StrokeFactory.
-}
